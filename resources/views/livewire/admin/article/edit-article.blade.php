<div class="relative bg-white rounded-xl mb-10 pt-2">
    ...
    <div class="py-10 px-4">
        ...
        <form action="post" wire:submit.preventDefault="update">
           ...
           <div x-data="{
                    ...
                    isOpen: false,
                    topicId: $wire.entangle('topic_id').live,
                    newTopic: $wire.entangle('new_topic').live,
                        ...
                        <ul class="list-none divide-y divide-gray-200 bg-white shadow rounded">
                            <template x-for="option in options" :key="option">
                                <li x-text="option" class="py-1.5 px-2 cursor-pointer hover:bg-gray-300"
                                    x-on:click="selected = option; isOpen = false ; topicId = null; newTopic = null;">
                                </li>
                            </template>
                        </ul>
                    </div>
                ...
                <div class="w-full md:w-[70%]">
                    <template x-if="selected==='Create new topic' ">
                        <div class="w-full">
                            <x-label for="new_topic" class="block w-full text-md">Topic title</x-label>
                            <x-input type="text" class="w-full" wire:model.live="new_topic"
                                placeholder="Enter new topic title" />
                            <x-input-error for="new_topic" />
                        </div>
                    </template>
                        ...
                        <div class="w-full">
                            ...
                            <x-select class="w-full mt-2" wire:model.live="topic_id">
                                ...
                                @foreach ($allTopics as $item)
                                    <option value="{{ $item->id }}" @if($topic_id==$item->id) selected @endif>{{ $item->title }}</option>
                                @endforeach
                            </x-select>
            ...
            <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
                ...
                @else
                <div x-show="! uploading" class="flex flex-col items-center w-40">
                    <span class="text-sm mb-1">Current image</span>
                    <img src="{{ asset('storage/'.$article->image) }}" alt="current photo"
                        class="w-full h-40 rounded-lg">                    
                </div>         
                @endif
                ...
                @foreach ($content_blocks as $i => $block)
                    <div class="w-full py-6 border-b first:border-y border-gray-200">
                        ...
                        @elseif($block['type'] == 'image')
                            <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
                                ...
                                <label for="image"
                                    class="rounded-xl text-primary-50 bg-slate-800 px-5 py-2.5 transition hover:-translate-y-1">choose
                                    photo</label>
                                @if ($block['content'] !== '' && $block['content']->temporaryUrl())
                                    <div class="flex flex-col items-center w-40">
                                ...
                                <div x-show="! uploading" class="flex flex-col items-center w-40">
                                    <span class="text-sm mb-1">Current image</span>
                                    <img src="{{ asset('storage/'.$block['content']) }}" alt="current photo"
                                        class="w-full h-40 rounded-lg">                    
                                </div> 
                                @endif
                        ...
                        @elseif($block['type'] == 'ordered-list')
                            <div x-data="{
                                itemsCount: $wire.content_blocks[{{ $i }}]['content'].length,
                                ...
                                <div class="flex flex-wrap items-center gap-3">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span class="text-xs">(clicking 'set' will reset the list)</span>
                                    </div>
                        ...
                        @elseif($block['type'] == 'table')
                            <div x-data="{
                                ...
                                setStructure() {
                                    ...
                                    this.table.head.data = this.table.head.hasData ? Array.from({ length: this.table.columns }, () => '') : [];
                            
                                    // Update table rows and columns
                                    this.table.data = Array.from({ length: this.table.rows }, () => Array.from({ length: this.table.columns }, () => ''));
                                }
                            }">
                                ...
                                <p class="text-sm text-rose-500 mt-1.5 mb-1">Clicking 'SET' will reset the table</p>
            ...
            <div class="flex flex-wrap items-center justify-end gap-2.5 pt-3 mt-4 border-t border-gray-300">
                <x-action-message on="updated">Article updated</x-action-message>
                <button type="submit"
                    class="rounded-xl bg-blue-500 text-primary-50 px-5 py-2.5 transition hover:-translate-y-1">update</button>
            </div>
        </form>
    </div>
</div>
