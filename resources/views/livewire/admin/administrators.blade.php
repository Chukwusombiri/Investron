    ...
    <div class="flex-none w-full max-w-full px-3">
        <div
            ...
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between items-center flex-wrap">
                ...
                <a href="{{ route('admin.administrator.new') }}" class="px-8 py-2 font-bold leading-normal text-center text-white align-middle transition-all ease-in border-0 rounded-lg shadow-md cursor-pointer text-xs bg-slate-700 lg:block tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">create new</a>
            </div>
            <div class="my-10 w-full px-4 flex items-center">
                <div class="w-full relative border h-12 shadow p-4 rounded-full flex items-center">
                    <input type="text" wire:modellive="search" class="w-full appearance-none border-none outline-none focus:outline-none focus:border-none focus:ring-transparent" placeholder="Filter by Name, email">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td
                                    ...
                                    <div class="flex px-2 py-1">
                                        <div>
                                            ...
                                            <img src="{{asset('storage/'.auth('admin')->user()->profile_photo_path)}}" class="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-12 w-12 rounded-xl" alt="user1" />
                                            @else
                                            <img src="{{asset('storage/admin-photos/admin.jpg')}}" class="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-12 w-12 rounded-xl" alt="user1" />
                                            @endif
                                        </div>
                                ...
                                <td
                                    ...
                                    <span class="text-xs font-semibold leading-tight text-sky-600">Not suspended</span> 
                                </td>
                                <td
                                    class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">                                           
                                    <span class="text-xs font-semibold leading-tight text-slate-400">{{date('M d, Y | h:i',strtotime(auth('admin')->user()->created_at))}}</span>                                            
                                </td>
                                <td
                                    class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                    <span class="text-xs font-semibold leading-tight text-slate-400">current date and time</span>
                                </td>
                                <td
                                    class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">                                    
                            ...
                            @if (count($admins) > 0)
                                @foreach ($admins as $admin)
                                    <tr>
                                        <td
                                            ...
                                            <div class="flex px-2 py-1">
                                                <div>
                                                    ...
                                                    <img src="{{asset('storage/'.$admin->profile_photo_path)}}" class="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-12 w-12 rounded-xl" alt="user1" />
                                                    @else
                                                    <img src="{{asset('storage/admin-photos/admin.jpg')}}" class="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-12 w-12 rounded-xl" alt="user1" />   
                                                    @endif
                                                </div>
                                        ...
                                        <td
                                            class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">                                            
                                            <span class="bg-gradient-to-tl from-slate-600 to-slate-300 px-4 text-xs rounded-1.8 py-2 rounded-xl inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">{{str_replace('_',' ',$admin->admin_role)}}</span>                                            
                                        </td>
                                        <td
                                            class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            @if ($admin->isSuspended)
                                            <span class="text-xs font-semibold leading-tight text-rose-600">Suspended</span>
                                            @else
                                            <span class="text-xs font-semibold leading-tight text-sky-600">Not suspended</span>  
                                            @endif
                                        </td>
                                        <td
                                            class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">                                           
                                            <span class="text-xs font-semibold leading-tight text-slate-400">{{date('M d, Y | h:i',strtotime($admin->created_at))}}</span>                                            
                                        </td>
                                        <td
                                            class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <span class="text-xs font-semibold leading-tight text-slate-400">{{$admin->updated_at->diffForHumans()}}</span>
                                        </td>
                                        <td
                                            class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">                                            
