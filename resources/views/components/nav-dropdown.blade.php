...
<div x-data="{ open: false }" @click.away="open = false" @close.stop="open = false">
    <div @click="open = ! open" class="flex items-center">
        {{ $trigger }}
        <span class="ml-1.5" :class="{'-rotate-180':open , }">
    ...
    <div x-show="open"
            ...
            class="absolute top-100 h-96 left-0 w-full z-50 mt-6 rounded-md shadow-lg"
            style="display: none;"
            @click="open = false">
        <div class="h-full rounded-md ring-1 ring-black ring-opacity-5">
            {{ $content }}                     
        </div>
    </div>
</div>