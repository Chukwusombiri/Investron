...
class CreateArticle extends Component
{
    ...
    {              
        $validated = $this->validate([
            ...
            'topic' => [Rule::excludeIf($this->topic_id!==null),'string','unique:topics,title'],
            'topic_id' => [Rule::excludeIf($this->topic!==null),'integer','exists:topics,id'],
            ...
            'content_blocks.*.content' => ['required',function ($attribute, $value, $fail) use ($request) {
                ...
                if ($index !== null && $request->input("content_blocks.$index.type") === 'image') {
                    if (!is_file($value) || !in_array($value->getMimeType(), ['image/jpeg', 'image/png','image/jpg'])) {
                        $fail('The ' . $attribute . ' must be a valid image (jpeg, png, jpg).');
                    }
    
                    if ($value->getSize() > 1024 * 1024) { // 1 MB
                        $fail('The ' . $attribute . ' must not exceed 1MB.');
                    }
                }
            },],
        ...
        try {
            ...
            $this->dispatch('created successfully');
            $this->reset();
        } catch (\Throwable $th) {
            throw $th;
            Log::error('Unable to create article: ' . $th->getMessage());
            session()->flash('error', 'Unable to create article! Contact site manager');
        }
    }
